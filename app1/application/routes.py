...
from application.models import dateTable
import requests
...
def home():
    return render_template('display.html', title='Home')
...
def generate():
    ...
    data= dateTable(dateGenerate=str(response2.text+" "+response1.text), share=str(reponse3.text))
    db.session.add(data)
    db.session.commit()
